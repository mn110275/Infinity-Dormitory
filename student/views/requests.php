...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'send_problem') {
    try {
        ...
        if (empty($title)) throw new Exception('Vui lòng nhập tiêu đề báo cáo');
...
try {
    if ($student) {
        ...
        while ($row = mysqli_fetch_assoc($result)) { $problems[] = $row; }
    }
} catch (Exception $e) { }
?>
                ...
                <form method="POST" id="problemForm">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" id="title" class="form-control" 
                               required placeholder="VD: Hỏng vòi nước, Cháy bóng đèn...">
                    </div>

                    <div class="form-group">
                        <label for="content">Mô tả chi tiết</label>
                        <textarea name="content" id="content" class="form-control" rows="6" 
                                  maxlength="255" placeholder="Mô tả cụ thể tình trạng (không bắt buộc)..."></textarea>
                    </div>

                    <div class="form-actions">
                        <button type="button" class="btn btn-secondary" onclick="document.getElementById('problemForm').reset()">Xóa form</button>
                            ...
                            <div class="problem-item" id="problem-<?= $pr['PR_ID'] ?>">
                                ...
                                <div class="noti-actions">
                                    <button type="button" class="btn-danger-sm" 
                                            onclick="ProblemManager.revoke(<?= $pr['PR_ID'] ?>)">
                                        Thu hồi
                                    </button>
                                </div>
                            </div>
