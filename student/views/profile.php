      ...
      <div class="profile-main">
        <form method="POST" enctype="multipart/form-data" id="profileForm">
          ...
          <div class="form-section">
            ...
            <div class="form-grid">
              ...
              <div class="form-group">
                ...
                <input type="date" class="form-control" value="<?= date('Y-m-d', strtotime($student['STD_DOB'])) ?>" disabled>
              </div>

              <div class="form-group">
                <label>Phòng ở</label>
                <input type="text" class="form-control" value="<?= htmlspecialchars($student['ROOM_ID'] ?? 'Chưa sắp xếp') ?>" disabled>
              </div>
              
              <div class="form-group">
                <label>Tòa</label>
                <input type="text" class="form-control" value="<?= htmlspecialchars($student['BLOCK_ID'] ?? 'Chưa sắp xếp') ?>" disabled>
              </div>
            </div>
          </div>

          <div class="form-section">
            <h3>Thông tin liên hệ (Có thể chỉnh sửa)</h3>
            
            <div class="form-grid">
              <div class="form-group">
                <label for="std_phone">Số điện thoại *</label>
                <input type="tel" name="std_phone" id="std_phone" class="form-control" 
                       value="<?= htmlspecialchars($student['STD_PHONE']) ?>" 
                       required pattern="[0-9]{10,11}" placeholder="0901234567">
              </div>

              <div class="form-group full-width">
                <label for="std_adr">Địa chỉ *</label>
                <textarea name="std_adr" id="std_adr" class="form-control" rows="2" 
                          required placeholder="Nhập địa chỉ đầy đủ"><?= htmlspecialchars($student['STD_ADR']) ?></textarea>
              </div>
            </div>
          </div>
          ...
          <div class="form-section">
            ...
            <button type="button" class="btn btn-secondary" id="btnChangePassword">
              Đổi mật khẩu
            </button>
          </div>
...
<div id="passwordModal" class="modal" style="display: none;">
  <div class="modal-content">
    <div class="modal-header">
      <h3>Đổi mật khẩu</h3>
      <button type="button" class="modal-close" id="closePasswordModal">✕</button>
    </div>
