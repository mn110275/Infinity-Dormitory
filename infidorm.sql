DROP DATABASE IF EXISTS infidorm;
...
(
    ...
    BLOCK_STATUS ENUM('Active', 'Maintenance', 'Closed') DEFAULT 'Active'
);

CREATE TABLE SEMESTER (
    SEM_ID VARCHAR(50) NOT NULL PRIMARY KEY, 
    STARTDATE DATE NOT NULL,
    ENDDATE DATE NOT NULL,
    SEM_STATUS ENUM('Upcoming', 'Active', 'Archived') DEFAULT 'Upcoming'
);
...
(
    ...
    BLOCK_ID VARCHAR(10),
    ROOM_STATUS ENUM('Active', 'Maintenance', 'Closed') DEFAULT 'Active',
...
CREATE TABLE CONTRACT (
    ...
    STATUS ENUM('Active', 'Expired', 'Terminated', 'Upcoming') DEFAULT 'Active',
    PRIMARY KEY (STD_ID, SEM_ID),
...
BEGIN
    ...
    IF NEW.STATUS = 'Active' THEN
        SELECT OCCUPIED, CAPACITY INTO cur, cap FROM ROOM 
        WHERE ROOM_ID = NEW.ROOM_ID AND BLOCK_ID = NEW.BLOCK_ID;
        IF cur >= cap THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Phòng đã đủ người';
        END IF;
    END IF;
END$$

CREATE TRIGGER trg_contract_insert_after
...
BEGIN
    DECLARE cur INT;
    DECLARE cap INT;
    IF (NEW.STATUS = 'Active' AND (OLD.STATUS <> 'Active' OR OLD.ROOM_ID <> NEW.ROOM_ID OR OLD.BLOCK_ID <> NEW.BLOCK_ID)) THEN
        SELECT OCCUPIED, CAPACITY INTO cur, cap FROM ROOM 
        WHERE ROOM_ID = NEW.ROOM_ID AND BLOCK_ID = NEW.BLOCK_ID;
...
BEGIN
    ...
    IF OLD.STATUS = 'Active' AND NEW.STATUS <> 'Active' THEN
        UPDATE ROOM SET OCCUPIED = OCCUPIED - 1 
    ...
    -- TH3: Đang Active nhưng đổi phòng
    ELSEIF OLD.STATUS = 'Active' AND NEW.STATUS = 'Active' AND 
           (OLD.ROOM_ID <> NEW.ROOM_ID OR OLD.BLOCK_ID <> NEW.BLOCK_ID) THEN
        UPDATE ROOM SET OCCUPIED = OCCUPIED - 1 WHERE ROOM_ID = OLD.ROOM_ID AND BLOCK_ID = OLD.BLOCK_ID;
        UPDATE ROOM SET OCCUPIED = OCCUPIED + 1 WHERE ROOM_ID = NEW.ROOM_ID AND BLOCK_ID = NEW.BLOCK_ID;
    END IF;
END$$
...
('20252', '2026-02-01', '2026-06-30', 'Upcoming');
...
INSERT INTO STUDENT
(STD_ID, STD_NAME, STD_DOB, STD_GD, STD_PHONE, STD_ADR, USER_ID)
VALUES
('20230002', 'Lê Văn C', '2003-08-10', 'Nam', '0977123456', 'Thái Bình', 4),
('20210001', 'Phạm Minh Tuấn', '2003-05-10', 'Nam', '0987654321', 'Hà Nội', 5),
('20210005', 'Trần Văn Đức', '2003-06-30', 'Nam', '0987654325', 'Nghệ An', 9),
('20210006', 'Trần Văn Đức', '2003-06-30', 'Nam', '0987654325', 'Nghệ An', 10),
('20230001', 'Trần Thị B', '2004-03-15', 'Nữ', '0987654321', 'Nam Định', 3),
('20210002', 'Lê Thị Hoa', '2003-08-15', 'Nữ', '0987654322', 'Hải Phòng', 6),
('20210003', 'Hoàng Văn Nam', '2003-02-20', 'Nam', '0987654323', 'Nam Định', 7),
('20210004', 'Nguyễn Thị Lan', '2003-11-05', 'Nữ', '0987654324', 'Thanh Hóa', 8);

INSERT INTO CONTRACT
(STD_ID, SEM_ID, BLOCK_ID, ROOM_ID)
VALUES
('20230002', '20251', 'A', '101'),
('20210001', '20251', 'A', '101'),
('20210005', '20251', 'A', '102'),
('20210006', '20251', 'A', '102'),
('20230001', '20251', 'A', '201'),
('20210002', '20251', 'B', '201'),
('20210003', '20251', 'B', '102'),
('20210004', '20251', 'B', '201');

INSERT INTO STUDENT 
(STD_ID, STD_NAME, STD_DOB, STD_GD, STD_PHONE, STD_ADR, USER_ID) 
VALUES
('20210010', 'Nguyễn Hoàng Long', '2003-01-15', 'Nam', '0912345678', 'Hải Phòng', 11),
('20210011', 'Đặng Thu Thảo', '2003-12-20', 'Nữ', '0922334455', 'Nam Định', 12),
('20210012', 'Ngô Phương Anh', '2003-11-05', 'Nữ', '0933445566', 'Hà Nam', 13),
('20210013', 'Lê Thị Mai', '2003-05-25', 'Nữ', '0944556677', 'Thanh Hóa', 14),
('20210014', 'Bùi Bích Phương', '2003-09-12', 'Nữ', '0955667788', 'Quảng Ninh', 15),
('20210015', 'Trịnh Đình Quang', '2003-02-28', 'Nam', '0966778899', 'Phú Thọ', 16),
('20210016', 'Vũ Anh Tuấn', '2003-07-07', 'Nam', '0977889900', 'Bắc Ninh', 17),
('20210017', 'Đỗ Hùng Dũng', '2003-10-10', 'Nam', '0988990011', 'Gia Lai', 18),
('20210018', 'Hoàng Văn Thái', '2003-03-03', 'Nam', '0999001122', 'Lạng Sơn', 19),
('20210019', 'Nguyễn Khánh Ly', '2003-04-14', 'Nữ', '0900112233', 'Đà Nẵng', 20);

INSERT INTO CONTRACT
(STD_ID, SEM_ID, BLOCK_ID, ROOM_ID)
VALUES
('20210010', '20251', 'A', '101'),
('20210011', '20251', 'A', '201'),
('20210012', '20251', 'A', '201'),
('20210013', '20251', 'A', '301'),
('20210014', '20251', 'A', '310'),
('20210015', '20251', 'A', '401'),
('20210016', '20251', 'A', '405'),
('20210017', '20251', 'A', '415'),
('20210018', '20251', 'A', '102'),
('20210019', '20251', 'A', '305');
